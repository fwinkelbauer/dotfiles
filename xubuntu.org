...
chunkyard store --repository "$local" --paths "${paths[@]}" --includes "${includes[@]}" --prompt 'Store' "$@"
...
for remote in "${remotes[@]}"; do
    ...
    chunkyard keep --repository "$remote" --latest '200' --prompt 'Store'
done

echo "Cleaning up: $local"
chunkyard keep --repository "$local" --latest '100' --prompt 'Store'
#+end_src
