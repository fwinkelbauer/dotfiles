...
xfconf-query -c 'xsettings' -p '/Net/ThemeName' -n -s 'Numix'
xfconf-query -c 'xsettings' -p '/Net/IconThemeName' -n -s 'Numix-Circle'
xfconf-query -c 'xfwm4' -p '/general/theme' -n -s 'Numix'
xfconf-query -c 'xfwm4' -p '/general/placement_ratio' -n -s 100
xfconf-query -c 'xfce4-keyboard-shortcuts' -p '/commands/custom/<Super>e' -n -s 'emacs'
...
xfconf-query -c 'xfce4-screensaver' -p '/saver/idle-activation/delay' -n -s 15
...
#+begin_src sh :tangle "~/.local/bin/pls" :shebang "#!/bin/bash -eu"
...
(setq package-selected-packages
      '(embark-consult embark consult vertico orderless powershell doom-modeline company magit color-theme-sanityinc-tomorrow))
#+end_src
