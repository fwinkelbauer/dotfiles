...
chunkyard store --repository "$local" --paths "${directories[@]}" --includes "${includes[@]}" "$@"
...
chunkyard restore --repository "$local" --directory ~/ "$@"
...
for remote in "${remotes[@]}"; do
    ...
    chunkyard copy --repository "$local" --destination "$remote"

    echo "Pulling from: $remote"
    chunkyard copy --repository "$remote" --destination "$local"

    echo "Cleaning up: $remote"
    chunkyard keep --repository "$remote" --latest '400'
done

echo "Cleaning up: $local"
chunkyard keep --repository "$local" --latest '200'
#+end_src
