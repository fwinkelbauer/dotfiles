...
(defun org-agenda-add-num-of-contents-lines ()
  ...
  (cl-loop with hd-marker
           ...
           (goto-char (cdr int))
           (insert
            (if (< 0 (org-agenda-count-lines (org-agenda-get-some-entry-text hd-marker (buffer-size))))
                " (*)"
              ""))))
...
Adding ~CUSTOM_ID~ to a heading makes it easier to link to it after an HTML
export. Let's define a few functions to generate a ~CUSTOM_ID~ when the org file
has the option ~auto-id:t~ set. This snippet is based on several blog posts,
including [[https://blog.phundrak.com/better-custom-ids-orgmode/][this one]]:
